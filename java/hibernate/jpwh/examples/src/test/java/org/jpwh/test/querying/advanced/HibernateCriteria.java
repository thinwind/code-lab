...
public class HibernateCriteria extends QueryingTest {
    ...
    public void foo() throws Exception {
        ...
        try {
            ...
            {
                List<Object[]> result = session.createCriteria(Bid.class, "b")
                        .createAlias("item", "i", JoinType.RIGHT_OUTER_JOIN)
                        .add(Restrictions.or(Restrictions.isNull("b.id"),
    ...
    public void executeQueries() throws Exception {
        ...
        try {
            ...
            {
                // Ordering
                List<User> users = session.createCriteria(User.class).addOrder(Order.asc("firstname"))
            ...
            {
                List<User> users = session.createCriteria(User.class)
                        .add(Restrictions.eq("homeAddress.city", "Some City")).list();
            ...
            {
                List<User> users = session.createCriteria(User.class)
                        .add(Restrictions.sqlRestriction("length({alias}.USERNAME) < ?", 8, StandardBasicTypes.INTEGER))
                        .list();
            ...
            {
                // Projection
                List<Object[]> result = session.createCriteria(User.class)
                        .setProjection(Projections.projectionList().add(Projections.property("id"))
                                .add(Projections.property("username")).add(Projections.property("homeAddress")))
                        .list();
            ...
            {
                List<String> result = session.createCriteria(Item.class)
                        .setProjection(
                                Projections.projectionList()
                                        .add(Projections.sqlProjection("NAME || ':' || AUCTIONEND as RESULT",
                                                new String[] { "RESULT" }, new Type[] { StandardBasicTypes.STRING })))
                        .list();
            ...
            {
                List<Object[]> result = session.createCriteria(User.class).setProjection(Projections.projectionList()
                        .add(Projections.groupProperty("lastname")).add(Projections.rowCount())).list();
            ...
            {
                List<Object[]> result = session.createCriteria(Bid.class).setProjection(Projections.projectionList()
                        .add(Projections.groupProperty("item")).add(Projections.avg("amount"))).list();
            ...
            {
                // Joins
                List<Bid> result = session.createCriteria(Bid.class).createCriteria("item") // Inner join
                        .add(Restrictions.like("name", "Fo", MatchMode.START)).list();
            ...
            {
                List<Bid> result = session.createCriteria(Bid.class).createCriteria("item") // Inner join
                        .add(Restrictions.isNotNull("buyNowPrice")).createCriteria("seller") // Inner join
                        .add(Restrictions.eq("username", "johndoe")).list();
...

            {
                List<Bid> result = session.createCriteria(Bid.class).createCriteria("item") // Inner join
                        ...
                        .add(Restrictions.and(Restrictions.eq("s.username", "johndoe"),
            ...
            {
                List<Item> result = session.createCriteria(Item.class).setFetchMode("bids", FetchMode.JOIN)
...

            {
                List<Item> result = session.createCriteria(Item.class)
                        .createAlias("bids", "b", JoinType.LEFT_OUTER_JOIN).setFetchMode("b", FetchMode.JOIN)
                        .createAlias("b.bidder", "bdr", JoinType.INNER_JOIN).setFetchMode("bdr", FetchMode.JOIN)
                        .createAlias("seller", "s", JoinType.LEFT_OUTER_JOIN).setFetchMode("s", FetchMode.JOIN).list();
...

            {
                ...
                List<Item> result = session.createCriteria(Item.class, "i")
                        .add(Subqueries.geAll(new BigDecimal(10), sq)).list();
...

            {
                 ...
                 * you to fine-tune the search. You want the case of the last name to be
                 * ignored, and a substring search, so "Doe", "DoeX", or "Doe Y" would match. In
                 * addition, the <code>User</code> class has a <code>boolean</code> property
                 * called <code>activated</code>. As a primitive, it can't be <code>null</code>
                 * and its default value is <code>false</code>, so Hibernate would include it in
                 * the search and only return users that aren't activated. As you want all
                 * users, you tell Hibernate to ignore that property when building the search
                 * query.
                 */
                ...
                /*
                 * The <code>Example</code> is added to a <code>Criteria</code> as a
                 * restriction.
                 */
...

            {
                ...
                List<Item> items = session.createCriteria(Item.class).add(exampleItem).createCriteria("seller")
        ...
        ExcludeBooleanExample(Object template) {
            super(template, new PropertySelector() {
                ...
                public boolean include(Object propertyValue, String propertyName, Type type) {
                    return propertyValue != null && !type.equals(StandardBasicTypes.BOOLEAN);
                }
            });
        }
    }
